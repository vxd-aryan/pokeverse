...
export default function RegionMasteryQuiz() {
  ...
  const fetchQuestion = async () => {
    ...
    try {
      const res = await fetch('https://pokeverse-backend1.onrender.com/api/quiz/region');
      const data = await res.json();
      setQuiz(data);
      
      // Pull the real-time identity string from PokéAPI
  ...
  const handleGuess = async (guess: string) => {
    ...
    // Determine the answer programmatically on the client side for visual validation sync
    const id = quiz?.pokemon_id || 1;
    const actualRegion = id <= 151 ? "Kanto" : id <= 251 ? "Johto" : "Hoenn";
    setCorrectAnswer(actualRegion);

    const isCorrect = guess === actualRegion;
    ...
    <div className="min-h-screen bg-[#0f172a] text-white p-6 flex flex-col items-center justify-center">
      <div className="max-w-2xl w-full">
        ...
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-black text-emerald-400 tracking-widest uppercase flex items-center gap-2">
            🗺️ Region Mastery
          </h1>
        ...
        <div className="bg-slate-800 border border-slate-700 p-6 rounded-3xl shadow-2xl mb-8 flex flex-col items-center">
          <h2 className="text-xl font-bold text-slate-300 mb-4 text-center">
            Which region was <span className="text-emerald-400">{pokemonName}</span> discovered in?
          </h2>
          <div className="bg-slate-900 rounded-2xl p-4 w-full flex justify-center border border-slate-700/50">
            <img 
              src={quiz.artwork_url} 
              alt={pokemonName} 
              className="w-48 h-48 object-contain drop-shadow-[0_0_15px_rgba(16,185,129,0.25)]"
            />
          </div>
        </div>
        ...
        {isRevealed && (
          <div className="flex justify-center animate-fade-in-up">
            <button 
              ...
              className="bg-gradient-to-r from-emerald-500 to-teal-600 text-slate-950 px-8 py-4 rounded-2xl font-black tracking-widest uppercase hover:scale-105 transition-transform shadow-xl shadow-emerald-500/20"
            >
              Next Expedition →
            </button>
          </div>
